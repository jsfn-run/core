...
s.innerHTML=n.innerHTML;s.type=n.type;t.push(s);n.remove();
  ...
  async executeLambda($request: IncomingMessage, $response: ServerResponse) {
    ...
    const { request, response } = await this.prepareInputAndOutput(
      $request,
      $response
    );
  ...
  async augmentResponse(response: Response) {
    ...
    response.send = (status: number | any, body?: any) =>
      this.writeResponse(response, status, body);
    response.reject = (message: string) =>
      this.writeResponse(
        response,
        400,
